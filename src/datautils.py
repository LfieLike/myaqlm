...
def get_red_pajama(nsamples, seqlen, tokenizer, eval_mode=False):
    ...
    return trainloader
def get_tiny_pajama(nsamples, seqlen, tokenizer, eval_mode=False, cache_dir='cache'):
    # 检查缓存目录是否存在，不存在则创建
    ...
    if os.path.isfile(cache_file):
        # 从硬盘加载数据
        trainloader = torch.load(cache_file)
    else:
        # 加载数据集
        ...
        for _ in range(nsamples):
            ...
            inp = trainenc.input_ids[:, i:j]
            tar = inp.clone()
            tar[:, :-1] = -100  # 使用-100做为标记，通常用于忽略计算损失
            trainloader.append((inp, tar))
        
        # 将处理后的数据缓存到硬盘
        torch.save(trainloader, cache_file)
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, eval_mode=False, model_path=None):
    ...
    if name.lower() == "none":
        ...
        if name.lower() == "wikitext2":
            ...
            data = get_tiny_pajama(nsamples, seqlen, tokenizer, eval_mode=eval_mode)
        else:
            raise ValueError(
