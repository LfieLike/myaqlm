...
from pq_engine import PQEngine
from src.pq import QuantizedLinear
...
def quantize_model(model, args):
    ...
    results = quantize_pq(model, dataloader, args)
...
@torch.no_grad()
def quantize_pq(model: PreTrainedModel, dataloader: Iterable, args: Namespace):
    assert not torch.backends.cuda.matmul.allow_tf32
    print("\nStarting PQ quantization ...")
    ...
    for layer_index in range(len(layers)):
        ...
        for names in sequential:
            if len(args.devices) == 1:
                ...
                pq_handlers = init_pq_engines_parallel(
                    args.devices,
                    layer,
                    [
                        name
            ...
            for sublayer_name in pq_handlers.keys():
                ...
                quantized_weight = pq_handlers[sublayer_name].quantize(args=args, verbose=True)

                with torch.no_grad():
                    assert pq_handlers[sublayer_name].layer.weight in set(
                        layer.parameters()
                    )  # test that this is not a replica

                    new_linear = QuantizedLinear(quantized_weight, pq_handlers[sublayer_name].layer.bias)
                    ...
                    for submodule in layer.modules():
                        for child_name, child_module in submodule.named_children():
                            if child_module is pq_handlers[sublayer_name].layer:
                ...
                overall_bits += int(weight_avg_bits * torch.numel(pq_handlers[sublayer_name].layer.weight.data))
                model_number_of_params += torch.numel(pq_handlers[sublayer_name].layer.weight.data)
...
def init_pq_engines(
    ...
    **forward_args: Dict[str, Any],
    ...
    """
    Create a dictionary of PQUtil instances for each quantized layer;
    Run forward pass on each sample in inps_tensor; write output activations to outs_tensor (in-plance)
    Accumulate XTX to each one of pq_handlers
    ...
    :returns: a dictionary where keys are full layer names and values are PQUtil instances ready to run .quantize
    """
    ...
    for sublayer_name in subset:
        pq_handlers[sublayer_name] = PQEngine(subset[sublayer_name])
    ...
    wrapped_layer_to_hander = {pq_handler.layer: pq_handler for pq_handler in pq_handlers.values()}
...
class _LayerWrapperThatAccumulatesXTX(nn.Module):
    def __init__(self, layer: nn.Module, pq_handler: PQEngine):
        super().__init__()
        self.wrapped_layer, self.pq_handler = layer, pq_handler

    def forward(self, input, *args, **kwargs):
        self.pq_handler.add_batch(input)
        return self.wrapped_layer(input, *args, **kwargs)


@torch.no_grad()
def init_pq_engines_parallel(
    ...
    **forward_args,
    ...
    """Parallel version of init_pq_engines; works on lists of input/output tensors"""
    layer_replicas = torch.nn.parallel.replicate(layer, devices=devices, detach=True)
    layer_replicas[0] = layer  # this ensures that pq_handlers returned by 0-th replica operate on the main layer
    funcs_by_device = [init_pq_engines for _ in devices]
    inputs_by_device = []
    kwargs_by_device = []
    ...
    pq_handles_by_device: Sequence[Dict[str, PQEngine]] = torch.nn.parallel.parallel_apply(
        funcs_by_device, inputs_by_device, kwargs_by_device, devices=devices
    )
    pq_handlers = pq_handles_by_device[0]
    for key, pq_handler in pq_handlers.items():
        replica_handlers = [device_pq_handlers[key] for device_pq_handlers in pq_handles_by_device]
        ...
        pq_handler.nsamples = total_nsamples
    return pq_handlers
